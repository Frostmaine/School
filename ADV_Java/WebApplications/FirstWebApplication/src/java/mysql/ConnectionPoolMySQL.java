 ...
 */
    private ConnectionPoolMySQL() {
        try {
            ...
            dataSource = (DataSource) ic.lookup("java:/comp/env/jdbc/mysql/MySQLDB");
        } catch (NamingException e) {
            PropertyManager.setProperty("UseDBPooling", "no");
            WebErrorLogger.log(Level.SEVERE,"Could not connect to our data source: "+
                    "java:/comp/env/jdbc/mysql/MySQLDB", e);
    //        utilities.Debug.println("Could not connect to our data source: "+"java:/comp/env/jdbc/mysql/MySQLDB");
        }
    }
