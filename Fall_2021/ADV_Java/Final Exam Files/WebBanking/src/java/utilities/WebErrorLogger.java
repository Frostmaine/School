...
import static utilities.PropertyManager.displayProperties;
//import org.apache.logging.log4j.web.WebLoggerContextUtils;

/**
 *  This logger is designed to place error messages into a database table and
 * display them if Debugging is enabled with the utilities.Debug class. 
 * 
...
public class WebErrorLogger {
     ...
     */
     public static void log(Level level, String message, Throwable ex) {
        if(servletContext != null){
           servletContext.log(message, ex);//Put the message into the Tomcat log
        } 
    ...
    public static void log(Level level, String message){
        if(servletContext != null){
           servletContext.log(message); //Put the message into the Tomcat log
        }
        int levelValue = level.intValue();
        if(logger!=null){
        ...
        if(Debug.emailErrorlMessage && levelValue>=Level.SEVERE.intValue()){
            ...
            if (useGmail!=null && useGmail.trim().equalsIgnoreCase("true")){
              GmailUtility.sendMail(adminEmail,"ErrorLogger" , 
                      "Error Message from a web application", message, false);
            }
            String useBloomMail = PropertyManager.getProperty("Use_BU_Mail");
            if (useBloomMail!=null && useBloomMail.trim().equalsIgnoreCase("true")){
                EmailUtility.email(adminEmail, message,"Error Message from a web application");
            }
        }
    }
}
